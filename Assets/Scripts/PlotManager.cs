...
public class PlotManager : MonoBehaviour {
    ...
    public class Plotter
    {
        ...
        public void Draw()
        {
            ...
            // Plot Data in buffer back from current position back to zero ----------------- modified
			...
			// Plot data in buffer from last position down to current position ----------------- modified
			if (dataFull)
            {
				for (int i = dataIndex; i <  gridWidth - 1; i++)
				{
					grid.SetPixel(x, data[i], plotColor);
                    x++;
                }
        ...
        {
            int x = this.Parent.grid.width - dataIndex;
